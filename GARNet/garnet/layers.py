...
class Encoder1(tf.keras.layers.Layer):
    ...
    def __init__(self, input_dim, output_dim, gene_size, adj, dropout, act=tf.nn.relu, featureless=False, **kwargs):
        super(Encoder, self).__init__(**kwargs)

        self.act = act
        self.adj = adj  # 将 adjacency_matrix 作为输入参数
        self.featureless = featureless
        self.dropout = dropout
        ...
        self.weight1 = self.add_weight(
            ...
            dtype=tf.float32,  # 确保权重是 float32 类型
            trainable=True,
            name="weight1"
        )

        self.weight2 = self.add_weight(
            shape=(gene_size, output_dim),
            initializer='glorot_uniform',
            dtype=tf.float32,  # 确保权重是 float32 类型
            trainable=True,
            name="weight2"
    ...
    def call(self, inputs):
        ...
        if not self.featureless:
            ...
            x = tf.cast(x, dtype=tf.float32)
            x = tf.nn.dropout(x, 1 - self.dropout)
            pre_sup = tf.matmul(x, self.weight1)  # 特征变换
        else:
            pre_sup = self.weight1

        # 变换并进行邻接矩阵乘法
        self.adj = tf.cast(self.adj, dtype=tf.float32) 
        T = tf.matmul(self.adj, pre_sup)  # 使用传入的邻接矩阵
        hidden = tf.add(T, self.weight2)
        return self.act(hidden)
...
class Decoder1(tf.keras.layers.Layer):
    ...
    def __init__(self, size1, latent_factor_num, act=tf.nn.sigmoid, **kwargs):
        super(Decoder, self).__init__(**kwargs)
        self.size1 = size1
        self.act = act  # 激活函数

        # 使用正确的方式初始化权重
        self.weight3 = self.add_weight(
            shape=(latent_factor_num, latent_factor_num),
            initializer=GlorotUniform(),  # 使用GlorotUniform初始化器
            trainable=True,
            name="weight3"
        )
    ...
    def call(self, hidden):
        """
        前向传播
        :param hidden: 输入隐层数据 (batch_size, num_nodes, latent_factor_num)
        :return: 解码后的矩阵
        """
        # hidden: (batch_size, num_nodes, latent_factor_num)
        # 权重变换
        M1 = tf.matmul(hidden, self.weight3)  # (batch_size, num_nodes, latent_factor_num)
        M1 = tf.matmul(M1, tf.transpose(hidden, perm=[0, 2, 1]))  # (batch_size, num_nodes, num_nodes)

        # 转换为向量
        M1 = tf.reshape(M1, [-1, 1])  # 将矩阵压平为列向量
